...
class VncViewModel(app: Application) : BaseViewModel(app) {
     ...
     * This app does not persist profiles, so this stays empty.
     */
    val savedProfiles: LiveData<List<ServerProfile>> = MutableLiveData(emptyList())
     ...
     * This app does not persist profiles, so this is a no-op.
     */
    fun saveProfile() {
    }

    suspend fun getProfileById(id: Long): ServerProfile? = null
    ...
    private inner class RemoteSessionObserver : RemoteSession.Observer {
        ...
        override fun onConnected(vncClient: VncClient, messenger: Messenger) {
            launchMain {
                ...
                // The first server update may arrive during the handshake, before the
                // client is marked connected. Request a fresh non-incremental frame now.
                refreshFrameBuffer()
                refreshFrameView()
        ...
        override fun onFramebufferSizeChanged(width: Int, height: Int) {
            // Frame updates can arrive before the main-thread callback runs. Keep the
            // framebuffer geometry ready before requesting the first GL draw.
