...
class Renderer(val viewModel: VncViewModel) : GLSurfaceView.Renderer {
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        if (state.fbWidth <= 0f || state.fbHeight <= 0f || state.vpWidth <= 0f || state.vpHeight <= 0f)
            return
        ...
        val drawCursor = !viewModel.pref.input.hideRemoteCursor && client.cursorInfo.width > 0
        if (drawCursor) {
            glEnable(GL_BLEND)
